...
def prepare_for_tsv_detection(predictions, dataset):
    ...
    for im_id, prediction in enumerate(predictions):
        image_info = dataset.get_img_info(im_id)  # TODO make sure this is dataset id and not image id
        if len(prediction) == 0:
            continue
        ...
        out_height, out_width = prediction.image_size
        sy, sx = image_height / out_height, image_width / out_width
        prediction.pred_boxes.scale(sy, sx)
        # prediction = prediction.resize((image_width, image_height))  # TODO check if above new scaling works
        # prediction = prediction.convert("xywh")
        prediction.pred_boxes.tensor = BoxMode.convert(prediction.pred_boxes.tensor, BoxMode.XYXY_ABS, BoxMode.XYWH_ABS)

        boxes = prediction.pred_boxes.tensor.tolist()
        scores = prediction.get("scores").tolist()
        labels = prediction.get("labels").tolist()
        if prediction.has_field("centers"):
            centers = prediction.get("centers")
        else:
            centers = None
...
def evaluate_box_proposals(
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        if prediction.has_field("objectness"):
            inds = prediction.get("objectness").sort(descending=True)[1]
        else:
            inds = prediction.get("scores").sort(descending=True)[1]
        ...
        gt_boxes = Boxes(BoxMode.convert(gt_boxes, BoxMode.XYWH_ABS, BoxMode.XYXY_ABS))
        ...
        overlaps = pairwise_iou(prediction, gt_boxes)
